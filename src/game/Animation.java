...
public class Animation extends GameObject {
	...
	public Animation(int x, int y, ID id) {
		...
		titleAnimation = loadImages(70, "titleAnimation");
		
		asteroidExplosionAnimation = loadImages(48, "explode");
		
		missleAnimation = loadImages(8, "missle");
		
		energyOrbAnimation = loadImages(25, "energy orb");
		
		gemAnimation = loadImages(6, "gem");
	...
	private BufferedImage[] loadImages(int imageAmount, String fileName){
		BufferedImage[] images = new BufferedImage[imageAmount];
		for (int i = 0; i < images.length; i++){
			
			try {
				images[i] = ImageIO.read( new File(fileName + "\\"+i+".png"));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
